...
import { NDropdown, NIcon, NModal, NButton, NSpace } from "naive-ui";
...
export default {
  ...
  methods: {
    ...
    handleImportSelect(key, option) {
      console.log("Import select key:", key, "option:", option);
      switch (key) {
        case "json":
          this.setJson();
          break;
    ...
    handleExportSelect(key, option) {
      console.log("Export select key:", key, "option:", option);
      switch (key) {
        case "json":
          this.showJson();
          break;
...
._fc-t-dialog .n-dialog__content {
  padding: 20px;
}

._fc-t-dialog .n-dialog {
  min-height: 500px;
}
